...
You can run the models on the Ryzen AI GPU in two simple steps:

**Model Conversion and Optimization**: After training the model, use the Microsoft Olive Optimizer to convert the model to ONNX and optimize it for target execution.

For additional information, refer to the `Microsoft Olive Documentation <https://microsoft.github.io/Olive/>`_


**Deployment**: After the model is in the ONNX format, use the ONNX Runtime DirectML EP (``DmlExecutionProvider``) to run the model on the AMD Ryzen AI GPU.
