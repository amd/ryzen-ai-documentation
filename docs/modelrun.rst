...
To avoid the overhead of recompiling models, you can save the compiled models and use them in the final application. You can load the pre-compiled models instantaneously and execute them immediately on the NPU. This greatly improves the session creation time and overall end-user experience.
...
|bulb| **TIP**: The VitisAI EP Cache mechanism is most convenient to quickly iterate during the development cycle. We recommend the OnnxRuntime EP Context Cache mechanism for the final version of the application.
...
The VitisAI EP includes a built-in caching mechanism. When you compile a model for the first time, the system automatically saves it in the VitisAI EP cache directory. When you create any ONNX Runtime session subsequently using the same model, the system loads the precompiled model from the cache directory. It reduces the session creation time.

Use the VitisAI EP Cache mechanism to quickly iterate during the development cycle. We do not recommend using it for the final version of the application.

Do not reuse cache directories that the Vitis AI Execution Provider generates across different versions of the Vitis AI EP or of the NPU drivers.

If using the VitisAI EP Cache, the application must check the version of the Vitis AI EP and of the NPU drivers. If the application detects a version change, it must delete the cache, or create a new cache directory with a different name.

The `cache_dir` and `cache_key provider` options specify the VitisAI EP cache location. For INT8 models, the :option:`enable_cache_file_io_in_mem` must be set to 0. Otherwise, the compiler output remains in memory and not saved to disk.
...
The previous example shows the cache directory set to the absolute path of the folder containing the executed script. After you create the session, the system saves the compiled model in the specified cache folder, inside a subdirectory named ``compiled_resnet50_int8``.
...
Use the following options for additional control:
...
By default, the generated context model is unencrypted and can be used directly during inference. If needed, encrypt the context model using one of the methods described in the following sections.

User-Managed encryption
~~~~~~~~~~~~~~~~~~~~~~~
After the process generates the context model, you can encrypt the generated file using a method of choice. At runtime, you must provide the same encryption key to decrypt and load the context model.

This method gives complete control you over the encryption process.

EP-Managed encryption
~~~~~~~~~~~~~~~~~~~~~~~
The VitisAI EP can optionally encrypt the EP context model using AES256. You can enable this by passing an encryption key using the :option:`encryption_key` VAI EP provider options. The key is a 256-bit value represented as a 64-digit string. At runtime, the same encryption key must be provided to decrypt and load the context model.

VitisAI EP seamlessly manages encryption and decryption with this method.
...
- Set the :option:`enable_cache_file_io_in_mem` provider option to 0
...
When these conditions are met, the system automatically generates the report file in the cache directory. This report includes information such as the following:
- The total number of nodes
- The list of operator types in the model
- Which nodes and operators run on the NPU or on the CPU.
